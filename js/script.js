let weatherAPIKey = "9ad1c67692c183670da0772bd9088968";
...
window.addEventListener("load", (e) => {
  ...
  let options = { enableHighFrequency: true, timeout: 5000, maximumAge: 0 };
  navigator.geolocation.getCurrentPosition(success, error, options);
  async function success(pos) {
    let crd = pos.coords;
    let lat = crd.latitude.toString();
    let lng = crd.longitude.toString();
    let endpoint = reverseGeoCodeEndpoint + "&lat=" + lat + "&lon=" + lng;
    let result = await fetch(endpoint);
...
let updateForeCast = (forecast) => {
  ...
  forecast.forEach((day) => {
    ...
    forecastItem += `<article class="weather_forecast_item">
          ...
          <span class="value">${temperature}</span> &deg;C</p>
      </article>`;
  });
